...
func main() {
	...
	// ipInfo, err := playwright.CheckIPAddress()
	// if err != nil {
	...
	if !skipDB {
		...
		if err := database.SaveNewsToDB(db, news); err != nil {
			log.Fatalf("Error saving news to database: %v", err)
		}
		log.Printf("Successfully saved %d news items to database", len(news))
	}
